...
export default new Vuex.Store({
  ...
  mutations: {
    ...
    handleDelete(state,_id) {
      state.noteList = state.noteList.filter((e) => {
        return e.id !== _id
      })
    },
    changeModel(state) {
      if (state.bgModel === 'dark') {
        state.bgModel = 'light'
      } else {
        state.bgModel = 'dark'
      }
    },
    changeModelShow(state,status) {
      state.modelShow = status
    }
  },
  actions: {},
  getters: {
